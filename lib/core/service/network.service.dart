...
class NetworkService {
  ...
  Future<MovieModelList> getMovies(String id, int no) async {
    ...
    var url = Uri.parse(
        'https://api.themoviedb.org/3/discover/movie?api_key=$apiKey&language=$language&sort_by=popularity.desc&with_companies=$id&page=$no');
  ...
  Future<TvModelList> getTvShows(String id, int no) async {
    ...
    var url = Uri.parse(
        'https://api.themoviedb.org/3/discover/tv?api_key=$apiKey&language=$language&sort_by=popularity.desc&with_networks=$id&page=$no');
    final response = await http.get(url);
    final data1 = json.decode(response.body);
    List<dynamic> list = data1['results'];
